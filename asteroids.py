...
class Ship:
    ...
    def vertices(self):
        ...
        vself = [[-self.base * 0.5, self.height * 0.5],
                 [self.base * 0.5, self.height * 0.5], [0, -self.height * 0.5]]
        vrot = self.rot(vself)
        v = self.bias(vrot)
        return v
...
class L_Asteroid(Asteroid):
    def __init__(self, x, y, speed, theta):
        super().__init__(x, y, speed, theta)
        self.size = 30
...
def main():
    ...
    for each in range(0, 5):
        Asteroids.append(Asteroid(random.randrange(0, w), random.randrange(
            0, h), speed=random.randrange(1, 10), theta=random.randrange(0, 360)))
    for each in range(0, 2):
        Asteroids.append(L_Asteroid(random.randrange(0, w), random.randrange(
            0, h), speed=random.randrange(1, 5), theta=random.randrange(0, 360)))
    Asteroids.append(Asteroid(x=639, y=479, speed=0,
    ...
    while not gameState == GAMEOVER:
        ...
        for event in pg.event.get():
            if event.type == pg.MOUSEBUTTONDOWN:
                ...
                Asteroids.append(L_Asteroid(pos[0], pos[1], speed=random.randrange(
                    1, 10), theta=random.randrange(0, 360)))
        ...
        if keys[pg.K_SPACE]:  # Shoot
            if len(bullets) <= MAX_BULLETS and sTimeOut < 0:
                # Not too many bullets
                bullets.append(
                    Bullet(ship.x, ship.y, ship.vx, ship.vy, ship.theta))
                sTimeOut = 3
        ...
        if keys[pg.K_b]:  # bomb
            if len(bombs) < MAX_BOMBS and bTimeOut < 0:  # Not too many bombs
                bTimeOut = 5
                bombs.append(
                    Bomb(ship.x, ship.y, ship.vx, ship.vy, ship.theta))
        sTimeOut = sTimeOut - 1
        bTimeOut = bTimeOut - 1
        ...
        for asteroid in Asteroids:
            ...
            for b in bombs:
                if b.dirtyCollDetect(asteroid):
                    ...
                    b.set_detonate(True)
                    break
            pg.draw.circle(screen, asteroid.color, [
...
# draw and move bombs
        for b in bombs:
            ...
            if b.detonate:
                ...
                pg.draw.line(screen, b.color, [b.x, b.y], [
                             b.tailx, b.taily], 6)
                if b.color == [75, 75, 75]:
                    b.color = [255, 0, 0]
                else:
...
# Draw and move ship
        ...
        if ship.shield:
            pg.draw.circle(screen, (255, 0, 0), [
                           int(ship.x), int(ship.y)], ship.size, 2)
